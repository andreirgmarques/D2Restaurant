...
function DataSetToJSON(ADataSet: TDataSet; AExcludeFields: array of string; ADataArrayName: string = 'data'): TJSONObject; overload;
...
{$ENDIF}
  ...
  try
   try
    Response := Client.Post(AURL{$IFnDEF FPC}, FileStream{$ENDIF});
   except
   end;

   vResp.FStatusCode:= {$IFnDEF FPC}Response.StatusCode{$ELSE}Client.ResponseStatusCode{$ENDIF};
   vResp.FStatusText:= {$IFnDEF FPC}Response.StatusText{$ELSE}Client.ResponseStatusText{$ENDIF};

   vResp.FContent:= Response.ContentAsString;
   vResp.FContentLength:= Length(vResp.FContent);


{$IFnDEF FPC}
   for var vHeader in Response.Headers do
    vResp.FHeaders.Values[vHeader.Name] := vHeader.Value;
{$ELSE}
   vResp.FHeaders.Text:= Client.ResponseHeaders.Text;
{$ENDIF}
...
{$IFnDEF FPC}
   for var vCookie in Response.Cookies do
    vResp.FCookies.Values[vCookie.Name] := vCookie.Value;
{$ELSE}
   vResp.FCookies.Text:= Client.Cookies.Text;
{$ENDIF}
...
{$IFnDEF FPC}
   vResp.FContentType:= Response.HeaderValue['Content-Type'];
{$ELSE}
   vResp.FContentType:= Client.ResponseHeaders.Values['Content-Type'];
{$ENDIF}
...
function DataSetToJSON(ADataSet: TDataSet; AExcludeFields: array of string; ADataArrayName: string): TJSONObject;
var
 vJSON: TJSONObject;
begin
    ...
    if Length(AExcludeFields) > 0 then
     ...
     if ADataSet.Fields[lColsArray].IsNull then
       case ADataSet.Fields[lColsArray].DataType of
         ...
         begin
          lJSONObject.{$IFnDEF FPC}AddPair{$ELSE}Add{$ENDIF}(lColName, TJSONNumber.Create(0));
         end;
         //string
...
{$ENDIF}
         ...
         ftBoolean:
           begin
             if ADataSet.Fields[lColsArray].AsBoolean then
               lJSONObject.{$IFnDEF FPC}AddPair{$ELSE}Add{$ENDIF}(lColName, {$IFnDEF FPC}TJSONTrue.Create{$ELSE}True{$ENDIF})
             else
               lJSONObject.{$IFnDEF FPC}AddPair{$ELSE}Add{$ENDIF}(lColName, {$IFnDEF FPC}TJSONFalse.Create{$ELSE}False{$ENDIF});
           end;
         // númericos
         ...
         begin
           if (ADataSet.Fields[lColsArray] as TNumericField).DisplayFormat <> '' then
            ...
            lJSONObject.{$IFnDEF FPC}AddPair{$ELSE}Add{$ENDIF}(lColName, {$IFnDEF FPC}TJSONNumber{$ELSE}TJSONFloatNumberFixed{$ENDIF}.Create(ADataSet.Fields[lColsArray].AsFloat));
         end;
         ftCurrency:
         begin
           if (ADataSet.Fields[lColsArray] as TCurrencyField).DisplayFormat <> '' then
            ...
            lJSONObject.{$IFnDEF FPC}AddPair{$ELSE}Add{$ENDIF}(lColName, {$IFnDEF FPC}TJSONNumber{$ELSE}TJSONFloatNumberFixed{$ENDIF}.Create(ADataSet.Fields[lColsArray].AsCurrency));
         end;
         ftSmallint{$IFDEF SUPPORTS_FTEXTENDED}, ftShortint, ftSingle{$ENDIF}, ftWord, ftInteger, ftAutoInc,
         ftLargeint{$IFDEF SUPPORTS_FTEXTENDED}, ftLongWord{$ENDIF}:
           lJSONObject.{$IFnDEF FPC}AddPair{$ELSE}Add{$ENDIF}(lColName, TJSONNumber.Create(Int64(ADataSet.Fields[lColsArray].Value)));
         //string
         ftString, ftFmtMemo, ftMemo, ftWideString, ftWideMemo, ftUnknown :
      ...
      begin
        ...
        case Field.DataType of
          ...
          ftLargeint {$IFnDEF FPC}, ftLongWord{$ENDIF}:
            Field.AsLargeInt := LValue.AsType<LargeInt>;

          // Float / Extended / BCD
          ftFloat, {$IFnDEF FPC}ftExtended, ftSingle,{$ENDIF} ftFMTBcd, ftBCD:
            Field.AsFloat := StrToFloat(LValue.AsType<String>, FormatSettings);

          ftCurrency:
            Field.AsCurrency := StrToFloat(LValue.AsType<String>, FormatSettings);

          // Boolean
          ftBoolean:
            Field.AsBoolean := SameText(LValue.AsType<String>, 'true') or (LValue.AsType<String> = '1');

          // Strings
          ftString, ftWideString, ftMemo, ftFmtMemo, ftWideMemo, ftUnknown:
            Field.AsString := LValue.AsType<String>;

          // Datas
          ftDate:
            Field.AsDateTime := StrToDate(LValue.AsType<String>, FormatSettings);

          ftTime:
            Field.AsDateTime := StrToTime(LValue.AsType<String>, FormatSettings);

          ftDateTime, ftTimeStamp:
            Field.AsDateTime := StrToDateTime(LValue.AsType<String>, FormatSettings);

          // Bytes (hex)
          ftBytes:
          begin
            SetLength(Bytes, Length(LValue.AsType<String>) div 2);
            P := 1;
            for B := 0 to High(Bytes) do
            begin
              Bytes[B] := StrToInt('$' + Copy(LValue.AsType<String>, P, 2));
              Inc(P, 2);
            end;
            Field.AsBytes := Bytes;
          end;
...
begin
 ...
 if AsJSONObject.FindValue(ADataArrayName) = nil then
  exit;

 if (AsJSONObject.FindValue(ADataArrayName) is TJSONArray) then
 begin
  vJSONArray:= AsJSONObject.FindValue(ADataArrayName) as TJSONArray;

  PopuleDataSet(ADataSet, vJSONArray);
 end else
 begin
  vJSONArray:= TJSONArray.Create;
  vJSONArray.Add(AsJSONObject.FindValue(ADataArrayName).Clone as TJSONObject);
...
begin
 ...
 begin
  vJSONArrayChild:= (vJSONArray[I] as TJSONObject).GetValue<TJSONArray>(ADataChildArrayName);
...
begin
 ...
 begin
  vJSONArrayChild:= (AsJSONArray[I] as TJSONObject).GetValue<TJSONArray>(ADataChildArrayName);
