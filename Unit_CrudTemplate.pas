...
uses
  ...
  D2Bridge.Item.HTML.Card, D2Bridge.Item.HTML.Row, D2Bridge.Forms;

type
  TCrudOperation   = (OpOpen = 0, OpSearch, OpInsert, OpEdit, OpSave, OpDelete, OpBack, OpClose);
  TCrudPermission  = (PermTotal = 0, PermSearch, PermEdit, PermInsert, PermSave, PermDelete);
  TCrudPermissions = set of TCrudPermission;

type
  TFormCrudTemplate = class(TD2BridgeForm)
  ...
  strict private
    ...
    FColAlign: string;
    // Active Operation
    FActiveOperation: TCrudOperation;
    // Set/Get Property
  ...
  private
    function CrudOperationPermission(ACrudOperation: TCrudOperation): boolean;
    procedure Crud_DoExportCardSearch; virtual;
    procedure Crud_DoExportCardData; virtual;
    function GetColAlign: string;
    procedure SetColAlign(const Value: string);
  public
    // Start
    ...
    procedure CrudDefaultSearch(ATable: string; AWhereFields: array of string; ASearch: string;
      AAnd: TArray<string>); overload;
    procedure CrudDefaultSearch(ATable: string; AWhereFields: array of string; ASearch: string; AAnd: TArray<string>;
      AOr: TArray<string>); overload;
    procedure CrudDefaultSearch(ATable: string; AWhereFields: array of string; ASearch: string; AAnd: TArray<string>;
      AOr: TArray<string>; AOrderFields: TArray<string>); overload;
  protected
    // Crud Operations
    procedure CrudOperation(ACrudOperation: TCrudOperation);
    // Crud Events
  ...
  published
    property ActiveOperation: TCrudOperation read FActiveOperation;
    property StartCard: TD2BridgeItemHTMLCard read FStartCard;
    property CrudColAlign: string read GetColAlign write SetColAlign;
...
uses Unit_DM, D2BridgeFormTemplate;
...
procedure TFormCrudTemplate.CrudDefaultSearch(ATable, AFieldWhere, ASearch, AFieldOrder: string);
begin
  CrudDefaultSearch(ATable, [AFieldWhere], ASearch, [], [], [AFieldOrder]);
end;

procedure TFormCrudTemplate.CrudDefaultSearch(ATable: string; AWhereFields: array of string; ASearch: string);
begin
...
procedure TFormCrudTemplate.CrudDefaultSearch(ATable: string; AWhereFields: array of string; ASearch: string;
  AAnd, AOr: TArray<string>);
begin
  CrudDefaultSearch(ATable, AWhereFields, ASearch, AAnd, AOr, []);
end;

procedure TFormCrudTemplate.CrudDefaultSearch(ATable: string; AWhereFields: array of string; ASearch: string;
  AAnd, AOr, AOrderFields: TArray<string>);
var
  I: integer;
...
procedure TFormCrudTemplate.CrudOperation(ACrudOperation: TCrudOperation);
var
  vActualOperation: TCrudOperation;
begin
  ...
  begin
    ...
    else if ACrudOperation in [OpSearch] then
    begin
      FActiveOperation := ACrudOperation;

      CrudOnSearch(Crud_EditSearch.Text);
    end
    else if ACrudOperation in [OpInsert] then
    begin
      vActualOperation := FActiveOperation;
      FActiveOperation := ACrudOperation;
      ...
      begin
        ...
        FActiveOperation := vActualOperation;
    end
    else if ACrudOperation in [OpEdit] then
    begin
      vActualOperation := FActiveOperation;
      FActiveOperation := ACrudOperation;
      if CrudOnEdit then
    ...
    else if ACrudOperation in [OpSave] then
    begin
      vActualOperation := FActiveOperation;
      FActiveOperation := ACrudOperation;
      if CrudOnSave then
    ...
    else if ACrudOperation in [OpDelete] then
    begin
      if MessageDlg('Do you confirm to delete this record?', mtconfirmation, [mbyes, mbno], 0) = mryes then
      begin
    ...
    else if ACrudOperation in [OpBack] then
    begin
      if FActiveOperation = OpInsert then
        if MessageDlg('Do you cancel this record?', mtconfirmation, [mbyes, mbno], 0) = mrNo then
          exit;
    ...
    else if ACrudOperation in [OpClose] then
    begin
      vActualOperation := FActiveOperation;
      FActiveOperation := ACrudOperation;
      if CrudOnClose then
...
function TFormCrudTemplate.CrudOperationPermission(ACrudOperation: TCrudOperation): boolean;
begin
  Result := True;

  if PermTotal in CrudPermissions then
    Result := True
  else if (ACrudOperation = OpSearch) and (not(PermSearch in CrudPermissions)) then
    Result := False
  else if (ACrudOperation = OpEdit) and (not(PermEdit in CrudPermissions)) then
    Result := False
  else if (ACrudOperation = OpInsert) and (not(PermInsert in CrudPermissions)) then
    Result := False
  else if (ACrudOperation = OpSave) and (not(PermSave in CrudPermissions)) then
    Result := False
  else if (ACrudOperation = OpDelete) and (not(PermDelete in CrudPermissions)) then
    Result := False
end;
...
//  SubTitle := 'Principal';
  ...
  begin
    with Row('d2bridgecrudtemplate ' + CrudColAlign).Items.Add do
...
begin
  ...
  FColSize        := 'col';
  FColAlign       := CSSClass.Col.Align.left;
...
function TFormCrudTemplate.GetColAlign: string;
begin
  Result := FColAlign;
end;
...
procedure TFormCrudTemplate.InitControlsD2Bridge(const PrismControl: TPrismControl);
begin
  inherited;
...
procedure TFormCrudTemplate.RenderD2Bridge(const PrismControl: TPrismControl; var HTMLControl: string);
begin
  inherited;
...
procedure TFormCrudTemplate.SetColAlign(const Value: string);
begin
  FColAlign := Value;
end;
